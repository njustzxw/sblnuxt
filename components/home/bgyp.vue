    ...
    <div class="box-hd">
        ...
        <div class="more clearfix">
            <ul class="clearfix fl tab">
                <li  v-for="(item,index) in bgypheadList" :key="index" class="fl" @mouseover="changecontent(item,index)" >
                    <span :class="bgypfag==index?'active':''">{{item}}</span>
                </li>
            </ul>
            <div class="fl">
                <span @click="goto1('/allProduct/all')" >查看更多</span>
            ...
            <div class="goods-list clearfix">
                <ul class="span-first fl">
                    <li class="brick-item brick-item-l">
                        <img src="http://image.99sbl.com/floor/5.jpg" alt="">
                    </li>
                </ul>
                <ul class="span-second clearfix fl">
                    <li v-for="(item,index) in bgypList" :key="index" class="common-item fl" @click="goto(item.id)" @mouseover="getList(item,index)" @mouseout="showprice=-1">
                        <div class="bcontent">
...
export default {
  data() {
    return {
      showprice: -1,
      bgypfag: 0,
      bgypheadList: [ "笔","鼓粉盒", "卫生用纸", "色带"],
      idarr: [10196,10187, 10200, 10191],
      bgypList: [],
      currentcatid: 10196,
      bgsb: []
    };
  ...
  mounted() {
    this.bgypList = this.bgsb;
    this.getproductList(10196);
  },
  methods: {
	goto1(url) {
    ...
    changecontent(item, index) {
      this.bgypfag = index;
      this.getproductList(this.idarr[index]);
    },
    getproductList(id) {
      var params = {
        categoryId: id,
