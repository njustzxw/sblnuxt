...
function setInput(selectDay) {
    ...
    var content = checkYear + splitString + checkMonth + splitString + selectDay;
    $('#txt_calendar').attr('value',content) 
    hidDate();
}
...
function showDate() {
    ...
    var x = document.getElementById('txt_calendar').offsetLeft + 250;
...
function createDate(thisYear, thisMonth) {
    ...
    //关闭日历显示
    createDoc += '<span id="closeDate" onClick="hidDate()" style="float: right;font-size: 25px;margin: -7px 24px 0 0;cursor: pointer;">×</span></p></div>';;
    ...
    //将创建好的控件字符串添加到div中  
    document.getElementById('date1').innerHTML = createDoc;
    //默认选择当前年份
